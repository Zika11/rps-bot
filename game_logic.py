...
import sqlite3
from models import DB_NAME, get_connection, create_tables

_initialized = False

# قفل للقنوات النشطة فقط
...
def get_or_create_user(user_id, name, username):
    uid = str(user_id)
    row = _fetchone("SELECT * FROM users WHERE user_id = ?", (uid,))
    if not row:
        u = {
...
def update_user(user_id, **kwargs):
    ...
    _execute(f"UPDATE users SET {set_clause} WHERE user_id = ?", values)

def get_user(user_id):
    uid = str(user_id)
    return _fetchone("SELECT * FROM users WHERE user_id = ?", (uid,))
...
def get_leaderboard(limit=10, period="all"):
    rows = _fetchall(f"SELECT * FROM users ORDER BY points DESC LIMIT ?", (limit,))
    return rows

# ─ـ العشائر ─ـ
def get_clan(clan_name):
    return _fetchone("SELECT * FROM clans WHERE clan_name = ?", (clan_name,))
...
def create_clan(clan_name, leader_id, description=""):
    ...
    _execute("INSERT OR IGNORE INTO clans VALUES (?,?,?,?,?)", (c["clan_name"], c["leader_id"], c["members"], c["points"], c["description"]))
...
def update_clan(clan_name, **kwargs):
    ...
    _execute(f"UPDATE clans SET {set_clause} WHERE clan_name = ?", values)

def get_all_clans():
    return _fetchall("SELECT * FROM clans ORDER BY points DESC")

def get_clans_count():
    row = _fetchone("SELECT COUNT(*) as cnt FROM clans")
    return row["cnt"] if row else 0

# ─ـ المهام ─ـ
def get_tasks(task_type=None):
    if task_type:
        return _fetchall("SELECT * FROM tasks WHERE type = ?", (task_type,))
    return _fetchall("SELECT * FROM tasks")

# ─ـ المتجر ─ـ
def get_shop_items():
    return _fetchall("SELECT * FROM shop")

# ─ـ التقييمات ─ـ
def add_rating(user_id, stars):
    _execute("INSERT OR REPLACE INTO ratings (user_id, stars) VALUES (?,?)", (str(user_id), stars))

def get_avg_rating():
    row = _fetchone("SELECT AVG(stars) as avg_rating, COUNT(*) as cnt FROM ratings")
    avg = round(row["avg_rating"], 1) if row["avg_rating"] else 0
    count = row["cnt"] if row else 0
    return avg, count

def get_user_rating(user_id):
    return _fetchone("SELECT * FROM ratings WHERE user_id = ?", (str(user_id),))

def get_user_by_username(username):
    """البحث عن مستخدم بالـ username (بدون @)"""
...
def create_tournament(tournament_id, prize=500):
    ...
    _execute("INSERT OR IGNORE INTO tournaments VALUES (?,?,?,?,?,?,?)", (t["tournament_id"], t["status"], t["players"], t["rounds"], t["winner_id"], t["prize"], t["created_at"]))
    return t

def get_active_tournament():
    rows = _fetchall("SELECT * FROM tournaments WHERE status IN ('open','running')")
    return rows[0] if rows else None

def get_tournament(tournament_id):
    return _fetchone("SELECT * FROM tournaments WHERE tournament_id = ?", (tournament_id,))
...
def join_tournament(tournament_id, user_id):
    ...
    _execute("UPDATE tournaments SET players = ? WHERE tournament_id = ?", (new_players, tournament_id))
    return True

def update_tournament(tournament_id, **kwargs):
    set_clause = ", ".join(f"{k} = ?" for k in kwargs)
    values = list(kwargs.values()) + [tournament_id]
    _execute(f"UPDATE tournaments SET {set_clause} WHERE tournament_id = ?", values)

# ─ـ الإنجازات ─ـ
def get_achievements():
    return _fetchall("SELECT * FROM achievements")
...
def add_achievement(user_id, ach_id):
    ...
    _execute("UPDATE users SET achievements = ? WHERE user_id = ?", (new_achievements, str(user_id)))
    return True

# ─ـ الأصدقاء ─ـ
def get_friends(user_id):
    rows = _fetchall("SELECT friend_id FROM friends WHERE user_id = ?", (str(user_id),))
    return [r["friend_id"] for r in rows]

def add_friend(user_id, friend_id):
    uid, fid = str(user_id), str(friend_id)
    _execute("INSERT OR IGNORE INTO friends VALUES (?,?)", (uid, fid))
    _execute("INSERT OR IGNORE INTO friends VALUES (?,?)", (fid, uid))

def remove_friend(user_id, friend_id):
    uid, fid = str(user_id), str(friend_id)
    _execute("DELETE FROM friends WHERE (user_id = ? AND friend_id = ?) OR (user_id = ? AND friend_id = ?)", (uid, fid, fid, uid))

def send_friend_request(from_id, to_id, from_name):
    _execute("INSERT OR IGNORE INTO friend_requests VALUES (?,?,?,?)", (str(from_id), str(to_id), from_name, str(datetime.now())))
    return True

def get_friend_requests(user_id):
    return _fetchall("SELECT * FROM friend_requests WHERE to_id = ?", (str(user_id),))

def remove_friend_request(user_id, from_id):
    _execute("DELETE FROM friend_requests WHERE to_id = ? AND from_id = ?", (str(user_id), str(from_id)))
...
def create_group_challenge(challenge_id, group_id, target_wins, prize, duration_hours=24):
    ...
    _execute("INSERT OR IGNORE INTO group_challenges VALUES (?,?,?,?,?,?,?,?)",
             (challenge_id, str(group_id), target_wins, prize, str(now), str(end), "{}", ""))

def get_active_group_challenge(group_id):
    rows = _fetchall("SELECT * FROM group_challenges WHERE group_id = ? AND winner_id = '' AND datetime(end_date) > datetime('now')", (str(group_id),))
    return rows[0] if rows else None

def update_group_challenge_participant(challenge_id, user_id, wins):
    challenge = _fetchone("SELECT * FROM group_challenges WHERE challenge_id = ?", (challenge_id,))
    if not challenge: return
    participants = _safe_json_load(challenge["participants"], {})
    participants[str(user_id)] = wins
    _execute("UPDATE group_challenges SET participants = ? WHERE challenge_id = ?", (json.dumps(participants), challenge_id))
    if wins >= challenge["target_wins"] and not challenge["winner_id"]:
        _execute("UPDATE group_challenges SET winner_id = ? WHERE challenge_id = ?", (str(user_id), challenge_id))

# ─ـ الألقاب والثيمات ─ـ
def get_titles_shop():
    return _fetchall("SELECT * FROM titles_shop")

def get_themes_shop():
    return _fetchall("SELECT * FROM themes_shop")

# ─ـ الأحداث الموسمية ─ـ
def get_active_event():
    rows = _fetchall("SELECT * FROM events WHERE datetime(start_date) <= datetime('now') AND datetime(end_date) >= datetime('now')")
    return rows[0] if rows else None

# ─ـ حرب العشائر ─ـ
def get_active_clan_war():
    rows = _fetchall("SELECT * FROM clan_wars WHERE datetime(start_date) <= datetime('now') AND datetime(end_date) >= datetime('now')")
    return rows[0] if rows else None
...
def add_clan_war_points(clan_name, points):
    ...
    _execute("UPDATE clan_wars SET clan_points = ? WHERE war_id = ?", (json.dumps(pts), war["war_id"]))
...
def create_clan_war(duration_days=7, prize_points=10000, prize_gems=50):
    ...
    _execute("INSERT INTO clan_wars VALUES (?,?,?,?,?)", (war_id, str(start), str(end), "{}", ""))
    return {"war_id": war_id, "prize_points": prize_points, "prize_gems": prize_gems}

def end_clan_war(war_id):
    war = _fetchone("SELECT * FROM clan_wars WHERE war_id = ?", (war_id,))
    if not war or war["winner_clan"]: return
    cp = _safe_json_load(war["clan_points"], {})
    if not cp: return
    winner = max(cp, key=cp.get)
    _execute("UPDATE clan_wars SET winner_clan = ? WHERE war_id = ?", (winner, war_id))
