...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # التصنيف
        ...
        await query.edit_message_text(text, reply_markup=back_btn("menu_rank"))
    ...
    # الأصدقاء (من handlers)
        ...
        context.user_data["awaiting"] = "add_friend"
        ...
        for req in requests:
            ...
            btns.append([InlineKeyboardButton(f"✅ قبول {req['from_name']}", callback_data=f"accept_friend_{req['from_id']}"),
                         InlineKeyboardButton(f"❌ رفض {req['from_name']}", callback_data=f"reject_friend_{req['from_id']}")])
        ...
        async with active_games_lock:
            ...
            active_games[game_id] = {"p1": user.id, "p1_name": user.first_name, "p2": friend_id,
                                     "p2_name": db.get_user(friend_id)["name"],
                                     "c1": None, "c2": None, "created_at": datetime.now(),
                                     "best_of": best_of, "p1_wins": 0, "p2_wins": 0, "game_type": "friend"}
        asyncio.create_task(game_timeout(game_id, context))
        accept_btn = InlineKeyboardMarkup([[InlineKeyboardButton("✅ قبول التحدي", callback_data=f"join_{game_id}")]])
        await context.bot.send_message(friend_id,
                                       f"⚔️ {user.first_name} بيتحداك من قائمة أصدقائه ({'أفضل من ٣' if best_of == 3 else 'جولة واحدة'})!\nاضغط قبول 👇",
    ...
    # العشائر
        ...
        if not clans:
            ...
            for i, c in enumerate(clans[:10]):
                ...
                members = len([m for m in c.get("members", "").split(",") if m])
                text += f"{medal} {c['clan_name']} — {c.get('points', 0)} نقطة — {members} عضو\n"
        await query.edit_message_text(text, reply_markup=back_btn("menu_clans"))
        ...
        db.update_clan(clan_name, members=",".join(members))
        db.update_user(user.id, clan=clan_name)
        await query.edit_message_text(f"✅ انضممت لعشيرة {clan_name}!", reply_markup=back_btn("menu_clans"))
        ...
        await query.edit_message_text(f"⚙️ إدارة عشيرة {clan_name}",
            reply_markup=InlineKeyboardMarkup([
    ...
    # لوحة المؤسس
        ...
        if not is_founder(user.id):
            ...
            return
        users_count = db.get_users_count()
        await query.edit_message_text(f"👑 لوحة المؤسس\n\n👥 إجمالي اللاعبين: {users_count}", reply_markup=founder_keyboard())
        ...
        await query.edit_message_text(text, reply_markup=back_btn("founder_panel"))

    elif data == "f_ratings":
        if not is_founder(user.id): return
        avg, count = db.get_avg_rating()
        await query.edit_message_text(f"⭐ التقييمات\n\nمتوسط: {avg}/50\nعدد التقييمات: {count}", reply_markup=back_btn("founder_panel"))
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting == "friend_challenge":
        ...
        if str(target_user["user_id"]) == str(user.id):
            await update.message.reply_text("❌ ما ينفعش تتحدى نفسك!")
            context.user_data["awaiting"] = None
            return
        best_of = context.user_data.get("friend_best_of", 1)
        async with active_games_lock:
            game_id = f"f_{user.id}_{random.randint(1000, 9999)}"
            active_games[game_id] = {"p1": user.id, "p1_name": user.first_name, "p2": None, "p2_name": None,
                                     "c1": None, "c2": None, "created_at": datetime.now(),
                                     "best_of": best_of, "p1_wins": 0, "p2_wins": 0, "game_type": "friend"}
        asyncio.create_task(game_timeout(game_id, context))
        accept_btn = InlineKeyboardMarkup([[InlineKeyboardButton("✅ قبول التحدي", callback_data=f"join_{game_id}")]])
        await context.bot.send_message(int(target_user["user_id"]),
                                       f"⚔️ {user.first_name} بيتحداك ({'أفضل من ٣' if best_of == 3 else 'جولة واحدة'})!\nاضغط قبول 👇",
                                       reply_markup=accept_btn)
        ...
        if not target_user:
            ...
            return
        if str(target_user["user_id"]) == str(user.id):
            await update.message.reply_text("❌ ما ينفعش تضيف نفسك!")
            context.user_data["awaiting"] = None
            return
        if db.send_friend_request(user.id, target_user["user_id"], user.first_name):
            ...
            try:
                await context.bot.send_message(int(target_user["user_id"]), f"📥 {user.first_name} بعتلك طلب صداقة!")
            except:
                pass
        else:
            await update.message.reply_text("❌ تم إرسال طلب بالفعل.")
        context.user_data["awaiting"] = None

    elif awaiting == "clan_name":
        if len(text) < 2 or len(text) > 20:
            await update.message.reply_text("❌ الاسم لازم بين 2 و 20 حرف!")
            return
        if db.get_clan(text):
        ...
        context.user_data["awaiting"] = None
        await update.message.reply_text(f"✅ تم إنشاء عشيرة {text}! 🗡️", reply_markup=main_menu_keyboard(user.id))
    ...
    # أوامر المؤسس
        ...
        if not text.isdigit():
            await update.message.reply_text("❌ ID رقمي.")
            return
        ...
        for uid in user_ids:
            try:
                await context.bot.send_message(int(uid), text, disable_notification=True)
                sent += 1
            except:
                pass
