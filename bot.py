...
# تسجيل حالة الاتصال بـ Redis و Google Sheets
if redis_client.is_connected():
    logger.info("✅ Redis connected successfully")
else:
    logger.warning("⚠️ Redis is disabled or not connected")

if gsheets.is_connected():
    logger.info("✅ Google Sheets connected successfully")
else:
    logger.warning("⚠️ Google Sheets is disabled or not connected")

# محاولة تحميل الإعدادات من Google Sheets
try:
    gsheet_settings = cache.get_settings()
    if gsheet_settings:
        logger.info(f"✅ Loaded {len(gsheet_settings)} settings from Google Sheets")
except Exception as e:
    logger.error(f"❌ Failed to load settings from Google Sheets: {e}")
    ...
    if chat_type == "private":
        ...
        if context.user_data.get("awaiting_set_points"):
            try:
                ...
                logger.error(f"Set points error: {e}")
