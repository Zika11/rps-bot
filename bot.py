...
pending_matches = []; active_games = {}
channel_tasks = {}; channel_games = {}; channel_last_play = {}
...
async def game_timeout(game_id, context):
    ...
    if game:
        try:
            ...
            if game.get("p2"): await context.bot.send_message(game["p2"], "⌛ انتهت صلاحية التحدي.")
        except: pass
...
async def check_and_complete_task(user_id, task_id, bot_context, progress_increment=1):
    ...
    if progress_data:
        ...
        except: progress = {"date": today, "tasks":{}}
    ...
    if task_def:
        ...
        if current >= required and not tasks_progress.get(f"{task_id}_done"):
            ...
            try: await bot_context.bot.send_message(user_id, f"🎉 أكملت مهمة *{task_def['description']}* وحصلت على {pts_reward} نقطة!", parse_mode="Markdown")
            except: pass
            rewarded = True
    ...
    for ach in all_achs:
        ...
        if current >= needed:
            if db.add_achievement(user_id, ach["ach_id"]):
                try: await context.bot.send_message(user_id, f"🎖️ إنجاز جديد: {ach['icon']} *{ach['name']}* - {ach['description']}", parse_mode="Markdown")
                except: pass
...
async def channel_loop(chat_id, context):
    while True:
        ...
        except Exception as e: logging.error(f"channel_loop {chat_id}: {e}")

async def cancel_channel_game(channel_id, context, reason=""):
    game = channel_games.pop(channel_id, None)
    if game:
        try: await context.bot.edit_message_text(chat_id=channel_id, message_id=game["message_id"], text=f"🚫 الجولة اتلغت{' - ' + reason if reason else ''}")
        except: pass
...
async def check_bot_permissions(chat_id, context):
    try:
        ...
        return True
    except tg_error.Forbidden: return False
    except Exception as e: logging.error(f"check_bot_permissions: {e}"); return False
...
# ─ـ وضع القصة ─ـ
STORY_LEVELS = {
    1: {"boss": "الرجل الحجري", "emoji": "🗿", "story": "في مملكة الأحجار، يهدد الرجل الحجري السلام..."},
    2: {"boss": "ملك الورق", "emoji": "📜", "story": "بعد هزيمة الرجل الحجري، يظهر ملك الورق..."},
    3: {"boss": "سيد المقصات", "emoji": "⚔️", "story": "أقوى زعيم في المملكة... هزمه يعني النصر!"}
}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        if arg.startswith("ref_"):
            ...
            #... (كود التحدي المباشر المختصر للاختصار)
            pass
        elif arg.startswith("joinclan_"):
            #... (كود الانضمام للعشيرة)
            pass
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # فردي
        ...
        if context.user_data.get("in_story_mode"):
            #... (معالجة تقدم القصة)
            pass
    ...
    # التصنيف
        ...
        await query.edit_message_text("🏆 اختار نوع التصنيف:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📅 يومي", callback_data="rank_daily"), InlineKeyboardButton("📆 أسبوعي", callback_data="rank_weekly")], [InlineKeyboardButton("🗓️ شهري", callback_data="rank_monthly"), InlineKeyboardButton("📊 إجمالي", callback_data="rank_all")], [InlineKeyboardButton("🔙 رجوع", callback_data="menu_main")]]))
    ...
    # المتجر
        ...
        owned = (u.get("shop_items","") or "").split(",")
        owned = [o for o in owned if o] + [item_id]
    ...
    # الأصدقاء
        ...
        for req in requests:
            text += f"• {req['name']}\n"
            btns.append([InlineKeyboardButton(f"✅ قبول {req['name']}", callback_data=f"accept_friend_{req['from']}"), InlineKeyboardButton(f"❌ رفض {req['name']}", callback_data=f"reject_friend_{req['from']}")])
        ...
        await query.edit_message_text("اختار نوع التحدي:", reply_markup=InlineKeyboardMarkup([
        ...
        active_games[game_id] = {"p1":user.id,"p1_name":user.first_name,"p2":friend_id,"p2_name":db.get_user(friend_id)["name"],"c1":None,"c2":None,"created_at":datetime.now(),"best_of":best_of,"p1_wins":0,"p2_wins":0,"game_type":"friend"}
        asyncio.create_task(game_timeout(game_id, context))
    ...
    # العشائر
        ...
        await query.edit_message_text(f"⚙️ *إدارة عشيرة {clan_name}*", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✏️ تعديل الوصف", callback_data=f"clan_desc_{clan_name}")], [InlineKeyboardButton("🔙 رجوع", callback_data=f"clan_view_{clan_name}")]]))
    ...
    # لوحة المؤسس
        ...
        users_count = len(db._cache["users"]); clans_count = len(db._cache["clans"])
        avg, count = db.get_avg_rating()
    ...
    # اللغة
        ...
        await query.edit_message_text("اختار اللغة:", reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🇪🇬 العربية", callback_data="lang_ar"), InlineKeyboardButton("🇬🇧 English", callback_data="lang_en")]
        ]))
    elif data.startswith("lang_"):
        lang = data.replace("lang_","")
        db.update_user(user.id, language=lang)
        await query.answer(_("language_changed", user.id), show_alert=True)
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting == "friend_challenge":
        ...
        if u := db.get_user(user.id):
            if u.get("clan"): await update.message.reply_text("❌ انت في عشيرة بالفعل!"); return
    ...
    # أوامر المؤسس
        ...
        if not text.isdigit(): await update.message.reply_text("❌ لازم تبعت ID رقمي."); return
        context.user_data["f_addpts_uid"] = int(text)
        context.user_data["awaiting"] = "f_addpts_amount"
        await update.message.reply_text("👌 تمام، دلوقتي ابعت عدد النقاط:")
